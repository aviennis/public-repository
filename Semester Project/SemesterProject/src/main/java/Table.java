...
public class Table {
	...
	protected Table(int columns, String name, ColumnDescription[] colDescriptions, ColumnDescription primary) throws IOException
	{
	 ...
	 * @throws IOException 
	 */
	protected boolean addRow(ColumnValuePair[] colValPairs) throws IOException
	{
	 ...
	 * @throws IOException
	 */
	protected ArrayList<DBEntry> select(SelectQuery sq) throws IOException
	{
					...
					else if(fi.toString().equals("MAX"))
					{
						if(isIndex[i])
						{
							...
							for(BTreeImplementation bti : BTrees)
							{
								...
								{
									max = (Integer)bti.getMaxKey(bti.root);
								}
							}
							returnList.add(max);
						}
					...
					else if(fi.toString().equals("MIN"))
					{
						if(isIndex[i])
						{
							...
							for(BTreeImplementation bti : BTrees)
							{
								if(bti.getName().equals(columnNames[i]))
								{
									min = (Integer)bti.getMinKey(bti.root);
								}
							}
							returnList.add(min);
						}
	 ...
	 * @throws IOException
	 */
	protected boolean createIndex(CreateIndexQuery ciq) throws IOException
	{
	 ...
	 * @throws IOException
	 */
	protected boolean deleteRow(DeleteQuery dq) throws IOException
	{
	 ...
	 * @throws IOException
	 */
	protected boolean updateQuery(UpdateQuery uq) throws IOException
	{
	 ...
	 * @param list
	 */
	protected void getDistinct(ArrayList<DBEntry> list)
	{
	...
	/**
	 * orders the list of rows. 
	 * NOTE: because I am using temporary BTrees to implement this, and because we were told to make 'null'
	 * the first key after the SENTIAL in the BTree, for my sorting, a null value will be at the beginning. 
	 * I'm not sure if this is ideal, but because we were told to make null the first value in the BTree, I 
	 * think logically its OK to make it the first value in the OrderBy as well
	 * @param list
	 * @param ob
	 * @return
	 */
		...
		for(int i = ob.length; i > 0; i--)
		{
			...
			for(int j = 0; j < amountOfColumns; j++)
			{
				...
				if(columnNames[j].equals(columnName))
				{
					...
					for(DBEntry dbe : list)
					{
						if(dbe.getEntry()[j].getValue() == null)
						{
							bti.add((Comparable) null, dbe);
						}
						else
						{
	...
	private void addToBTree(BTreeImplementation tree, int index, DBEntry entry)
	{
		if(types[index].toString().equals("INT"))
		{
			if(entry.getEntry()[index].getValue() != null)
			{
	...
	private boolean isUniqueValue(ColumnValuePair colValPair, int index) throws IOException
	{
		for(DBEntry dbe : rows)
		{
			if(!(dbe.getEntry()[index].getValue() == null))
			{
				if((dbe.getEntry()[index].getValue()).equals(stringToType(types[index], colValPair.getValue(), index)))
				{
					return false;
				}
			}
	...
	protected ArrayList<DBEntry> getTableList()
	{
		return rows;
	}
	...
	protected String[] getNameOfIndex()
	{
		return nameOfIndex;
	}
	...
	protected ArrayList<BTreeImplementation> getTrees()
	{
		return BTrees;
	}
